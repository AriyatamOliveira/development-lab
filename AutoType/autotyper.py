...
def human_typing(text):
    ...
    BASE_SPEED_MIN = 0.09  # Minimum delay between characters
    BASE_SPEED_MAX = 0.20  # Maximum delay between characters

    # Punctuation weight (additional pause after punctuation)
    PUNCTUATION_PAUSE_MIN = 0.4  # Minimum pause after punctuation
    PUNCTUATION_PAUSE_MAX = 0.7  # Maximum pause after punctuation

    # Mistake probability (chance of making a typing error)
    MISTAKE_PROBABILITY = 0.03  # 3% chance of making a mistake

    # Error correction timing (visible correction sequence)
    MISTAKE_OOPS_PAUSE = 0.15  # Pause after typing wrong character
    MISTAKE_BACKSPACE_PAUSE = 0.1  # Pause after backspacing

    # Buffer stability (mandatory cooldown after every keypress)
    KEYBOARD_COOLDOWN = 0.01  # Mandatory cooldown to prevent input lag
